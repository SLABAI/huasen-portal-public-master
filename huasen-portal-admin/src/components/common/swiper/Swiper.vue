...
<template>
  <div id="h-swiper">
    ...
    <div class="control-button">
      <span @click="previous">
        <i class="iconfont icon-vzuo"></i>
      </span>
      <span @click="next">
        <i class="iconfont icon-vyou"></i>
      </span>
    </div>
...
export default {
  ...
  mounted: function () {
    ...
    this.startTimer();
    // 修复屏幕大小改变导致轮播图滚动异常
    window.onresize = this.Tool.funcDebounce(() => {
  ...
  methods: {
    ...
    scrollContent: function (currentPosition) {
      ...
      this.swiperStyle.transition =
        "transform " + this.animationDuration + "ms"; // 设置动画时长
    ...
    touchEnd: function () {
      ...
      if (this.distance === 0) {
        ...
        this.distance > 0 &&
        currentMove > this.slideWidth * this.moveRatio
      ) {
